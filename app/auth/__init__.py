...
class UserSession(UserMixin):
    def __init__(self, user_model : User):
        assert isinstance(user_model, User) # It should be always an instance of the user
    ...
    def query_data(self):
        user_model = User.query.filter_by(id = self.id).first()
        return user_model
...
def load_user(user_id : str):
    ...
    user_model = UserSession.query_data_by_id(int(user_id))
